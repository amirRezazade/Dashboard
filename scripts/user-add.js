...
if (userId) {
  ...
  document.querySelector("#sidebar-edit-user-link").classList.add("text-[var(--active-color)]", "before:!bg-[var(--active-color)]");
  document.querySelector("title").textContent = "Dashboard || edit-user";
} else {
  document.querySelector("#sidebar-add-user-link").classList.add("text-[var(--active-color)]", "before:!bg-[var(--active-color)]");
  document.querySelector("title").textContent = "Dashboard || add-user";
}
...
async function submitUser() {
  showSwal(userId ? "ویرایش اطلاعات کاربر" : "اضافه کردن کاربر", userId ? "آیا میخواهید اطلاعات این کاربر را تغییر دهید؟" : "آیا میخواهید این کاربر را اضافه کنید؟", "warning", true, "تایید").then((data) => {
    if (data.isConfirmed) {
      fetch(`https://dummyjson.com/users/${userId ? userId : "add"}`, {
        ...
        .then((res) => res.json())
        .then(showTost("success", userId ? "مشخصات کاربر آپدیت شد!" : "کاربر اضافه شد!"))
        .catch((error) => {
          console.log(error);
...
function deleteUser() {
  showSwal("حذف کاربر", "آیا مطمئن هستید که می‌خواهید این کاربر را حذف کنید؟ این عمل قابل بازگشت نیست.", "warning", true, "تایید").then((e) => {
    if (e.isConfirmed) {
