...
let product;
const addToShoppingCardIdsBtn = document.getElementById(
  "add-to-shopping-card-btn"
);
const urlParams = new URLSearchParams(window.location.search);
const productId = urlParams.get("id");
let tabContent = document.querySelector("#tab-container");
productId ? "" : (window.location.search = "id=115");
document.querySelector("#product-edit-btn").href = `product-add.html?id=${
  productId || 115
}`;
document.querySelector("#product-delet-btn").addEventListener("click", () => {
  showSwal(
    "حذف محصول",
    ...
    fetch(`https://dummyjson.com/products/${productId || 115}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((response) => {
        ...
        } else showTost("error", "محصول حذف نشد دوباره امتحان کنید!!");
      });
  });
});
  ...
  try {
    ...
    let infos = `
            ...
            <div class="flex items-center justify-start gap-4 xs:gap-6 lg:gap-10">
              <div class=" "><i class="fa fa-star text-xs text-yellow-300"></i> <span class="font-[dana-num]">${
                product.rating
              }</span></div>
              <div><i class="fa fa-comment text-xs"></i> <span class="font-[dana-num]">${
                product.reviews.length
              }</span> نظر</div>
              <div><i class="fa fa-shopping-cart text-xs"></i> <span class="font-[dana-num]">${
                product.reviews.length * 2 + 4
              }</span> سفارش</div>
            </div>
            <div class="xs:w-1/2  lg:w-full 2xl:w-1/2 grid grid-cols-2  gap-y-1.5 items-center">
              ...
              <span>برند:</span>  <span  dir="ltr" class="text-end">${
                product.brand || "-----"
              }</span>
              <span>طول:</span>  <span class="font-[dana-num]">${
                product.dimensions.width || "-----"
              } سانتی متر</span>
              <span>عرض:</span>  <span class="font-[dana-num]">${
                product.dimensions.height || "-----"
              } سانتی متر</span>
              <span>ارتفاع:</span>  <span class="font-[dana-num]">${
                product.dimensions.depth || "-----"
              } سانتی متر</span>
              <span>وزن:</span>  <span class="font-[dana-num]">${
                product.weight || "-----"
              } کیلو گرم</span>
            </div>
            <div dir="ltr" class="border-t  border-gray-500/50 pt-3 flex flex-wrap  items-center gap-y-2 justify-end gap-x-6">
              <div ><i class="fa-solid fa-tag text-xs"></i> <span>${
                product.sku
              }</span></div>
              <div ><i class="fa-solid fa-award text-xs"></i> <span>${
                product.warrantyInformation
              }</span></div>
              <div ><i class="fa fa-truck text-xs"></i> <span>${
                product.shippingInformation
              }</span></div>
              <div ><i class="fa fa-rotate-left text-xs"></i> <span>${
                product.returnPolicy
              }</span></div>
            </div>
            <span>تعداد: <span class="font-[dana-num]">${
              product.stock
            }</span></span>
            <span>تخفیف: <span class="font-[dana-num]">${
              product.discountPercentage
            } %</span></span>
            <span>قیمت: <span class="font-[dana-num] text-base">${
              product.price
            } $</span></span>
    `;
    addPhotos(product.images);
    document
      .querySelector("#product-infos-container")
      .insertAdjacentHTML("afterbegin", infos);
    tabContent.textContent = product.description;
  } catch {
    document.querySelector(
      "section"
    ).innerHTML = `<img class="mx-auto" width="400" height="400" src="images/product-not-found.png" alt="">`;
  }
}
getProduct();
...
tabs.addEventListener("click", (e) => {
  if (e.target.nodeName === "BUTTON") {
    tabs
      ...
      .forEach((elem) => (elem.style.borderColor = "transparent"));
               ...
               <div class="flex flex-col text-sm">
                 <span>${el.reviewerName}</span>
                  <div class="flex items-center gap-0.5 text-[10px] text-amber-400">
                    <i class="${
                      el.rating < 1 ? "text-gray-500" : ""
                    } fa fa-star"></i>
                    <i class="${
                      el.rating < 2 ? "text-gray-500" : ""
                    } fa fa-star"></i>
                    <i class="${
                      el.rating < 3 ? "text-gray-500" : ""
                    } fa fa-star"></i>
                    <i class="${
                      el.rating < 4 ? "text-gray-500" : ""
                    } fa fa-star"></i>
                    <i class="${
                      el.rating < 5 ? "text-gray-500" : ""
                    } fa fa-star"></i>
                  </div>
               </div>
                             <span class="mr-auto text-xs text-left xs:hidden">${el.date.slice(
                               0,
                               10
                             )}</span>

              </div>
              <div dir="ltr" class="w-full my-1 text-end xs:text-center">${
                el.comment
              }</div>
              <span class="hidden  justify-self-end text-xs xs:block text-left w-full">${el.date.slice(
                0,
                10
              )}</span>
