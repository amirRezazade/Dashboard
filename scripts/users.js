...
async function getUsers(key) {
  ...
  let res = await fetch(`https://dummyjson.com/users${key ? `/search?q=${key}` : ""}${key ? "" : "?limit=150&select=id,image,firstName,lastName,gender,email,phone,age,username,role"}`);
...
function addUsers(list, page = 1) {
  ...
  if (list.length == 0) usersContainer.innerHTML = `<div class=" my-5 block h-auto max-h-[200px] sm:max-h-[350px] centered overflow-hidden"><img class="object-cover aspect-square max-w-sm sm:max-w-xl" src="images/item-notfound.png" alt="item-notfound"></div>`;
  ...
  copyList.splice(page * 10 - 10, 10).forEach((user) => {
    usersContainer.innerHTML += `
    <tr data-id="${user.id}" class="p-1.5 md:p-2 flex  transition-colors hover:bg-gray-500/20">
      <td class=" lg:pr-7 w-3/15">
        <a href="user-details.html?id=${user.id}" class="flex items-center gap-3.5">
          <div class="size-10 shrink0 rounded-full overflow-hidden"><img class="object-cover rounded-full" src="${user.image}" alt=""></div>
      ...
      <td class=" centered w-2/15 font-[dana-num] truncate" dir="ltr">${user.phone}</td>
      <td class=" centered w-1/15">${user.age}</td>
      <td dir="ltr" class=" centered w-2/15 "><span class="truncate">${user.username} </span></td>
      <td class=" flex items-center w-1/15">${user.role === "admin" ? "ادمین" : ""} ${user.role === "user" ? "مشتری" : ""} ${user.role === "moderator" ? "مدیر" : ""}</td>
      <td class=" centered w-1/15 gap-1.5 text-sm md:text-base">
          <button type="button" class="p-1 rounded-sm cursor-pointer text-blue-500" onclick="editUser(${user.id})"><i class="fa fa-edit"></i></button>
          <button type="button" class="p-1 rounded-sm cursor-pointer text-red-500" onclick="removeUser(${user.id})"><i class="fa fa-trash"></i></button>
      </td>
    </tr>
    `;
  });
...
function filtering() {
  ...
  let roleFiltering = [...users].filter((elem) => {
    return (user.checked && elem.role === "user") || (admin.checked && elem.role === "admin") || (moderator.checked && elem.role === "moderator");
  });
  if (orderingElem.value === "desc") {
    if (parameter === "age") {
      copyUsers = [...roleFiltering].sort((a, b) => a[parameter] - b[parameter]);
    } else {
      copyUsers = [...roleFiltering].sort((a, b) => a[parameter].localeCompare(b[parameter]));
    }
  } else {
    if (parameter === "age") {
      copyUsers = [...roleFiltering].sort((a, b) => b[parameter] - a[parameter]);
    } else {
      copyUsers = [...roleFiltering].sort((a, b) => b[parameter].localeCompare(a[parameter]));
    }
  }
...
function editUser(id) {
  ...
  Swal.fire({
    ...
    html: `
      ...
      <input id="firstName" type="text" required  class=" bg-[var(--bg-color)] p-3 outline-0 rounded-lg border border-gray-500/80 focus:border-[var(--active-color)]" value="${users[index].firstName}">
      <label class="mb-1 sm:mb-2 mt-2 sm:mt-4 cursor-pointer text-start" for="lastName">نام خانوادگی:</label>
      <input id="lastName" type="text" required  class=" bg-[var(--bg-color)] p-3 outline-0 rounded-lg border border-gray-500/80 focus:border-[var(--active-color)]" value="${users[index].lastName}">
      <label class="mb-1 sm:mb-2 mt-2 sm:mt-4 cursor-pointer text-start" for="username">نام کاربری:</label>
      <input id="username" type="text" required  class=" bg-[var(--bg-color)] p-3 outline-0 rounded-lg border border-gray-500/80 focus:border-[var(--active-color)]" value="${users[index].username}">
      <label class="mb-1 sm:mb-2 mt-2 sm:mt-4 cursor-pointer text-start" for="age">سن:</label>
      <input id="age" type="number" required min="18" max="100"  class=" bg-[var(--bg-color)] p-3 outline-0 rounded-lg border border-gray-500/80 focus:border-[var(--active-color)]" value="${users[index].age}">
      ...
      <select id="role" class=" border border-gray-500/80 focus:border-[var(--active-color)] outline-0 cursor-pointer w-full bg-[var(--bg-color)] p-2 md:p-3 rounded-lg ">
           <option ${users[index].role == "moderator" ? "selected" : ""} value="moderator">مدیر</option>
          <option ${users[index].role == "admin" ? "selected" : ""} value="admin">ادمین</option>
          <option ${users[index].role == "user" ? "selected" : ""} value="user">مشتری</option>
      </select>    
    </form>
    `,
    focusConfirm: false,
    ...
    if (result.isConfirmed) {
      showSwal("ویرایش اطلاعات", "آیا میخواهید اطلاعات این کاربر را تغییر دهید؟", "warning", true, "بله").then((res) => {
        if (res.isConfirmed) {
          fetch(`https://dummyjson.com/users/${id}`, {
            method: "PUT",
